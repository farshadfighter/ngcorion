/**
 * ManagePortsModal - Manage discovered ports with three modes
 *
 * Modes:
 * 1. Overwrite: Replace all existing port data with discovered data
 * 2. Update/Merge: Add discovered ports to existing ports
 * 3. Create: Add new asset with all discovered data
 */
...
const ManagePortsModal = ({ host, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    const checkForExistingAsset = async () => {
      try {
        ...
        // Note: Preview is only available if the host has an ID (from database)
        // Hosts from scan results don't have IDs yet, so we skip preview
        if (match && host.id) {
  ...
  const formatPorts = (ports) => {
    ...
    return ports.map(p => `${p.port}/${p.protocol} (${p.service || 'unknown'})`).join(', ');
  };
  ...
  const handleApplyMode = async () => {
    ...
    try {
      ...
      // For hosts with ID (from pending hosts), use the standard flow
      const payload = {
        hostId: host.id,
        mode: selectedMode,
      };
  ...
  const handleDirectApply = async () => {
    try {
      ...
      if (selectedMode === 'create_new') {
        ...
        alert(`Asset "${assetName}" created successfully!`);
        onClose();
