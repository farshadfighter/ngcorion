...
## ⚠️ وضعیت فعلی — ۲۰۲۶/۰۸/۰۹

تیم بک‌اند **۶ مورد از ۸ مورد** این سند را پیاده کرده است. متن بخش‌های بعدی
مربوط به وضعیت اولیه است و بخشی از آن دیگر معتبر نیست.

### ✅ حل شده

| # | مورد | نتیجه |
|---|---|---|
| ۱ | تکلیف `very_high` | سطح رسمی شد (`_RISK_LEVEL_ORDER`) |
| ۲ | `GET /api/risk/summary` | ساخته شد، تجمیع سمت SQL |
| ۳ | تعریف Non-conformity | `active_audit_findings_count > 0` |
| ۴ | ۶ فیلد در `_list_item` | هر شش فیلد اضافه شد |
| ۵ | `GET /api/risk/trend` | ساخته شد، تجمیع ماهانه |
| — | seed در startup | حالا در startup صدا زده می‌شود |
| — | نویسنده‌ی `asset_open_ports` | ماژول discovery به آن می‌نویسد |

**نکته:** چهار فیلد `*_findings_count` (بند ج مشکل ۴) در به‌روزرسانی قبلی جا
افتاده بود و توسط تیم فرانت اضافه شد — جزئیات در انتهای همین بخش.

### ⏳ باقی‌مانده

| # | مورد | تأثیر روی UI |
|---|---|---|
| ۶ | بلوک `hardening_impact` | بخش «Hardening Impact» — ستون «قبل» و هر سه progress bar |
...
## خلاصه‌ی مدیریتی (متن اولیه — تاریخی)
