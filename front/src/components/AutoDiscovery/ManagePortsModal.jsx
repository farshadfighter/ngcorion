...
const ManagePortsModal = ({ host, onClose, onSuccess }) => {
    ...
    // ✅ state برای کنترل alert
    const [alertInfo, setAlertInfo] = useState(null);
    // alertInfo = { message: '...', type: 'success' | 'error' | 'warning' | 'info' }

    // تابع کمکی جایگزین alert()
    ...
    const handleDirectApply = async () => {
        try {
            ...
            if (selectedMode === 'create_new') {
                ...
                // ✅ جایگزین alert
                showAlert(
                    `Ports ${selectedMode === 'overwrite' ? 'overwritten' : 'updated'} successfully!`,
                    'success'
                );
            }
        } catch (error) {
    ...
    return (
        <>
            {/* ✅ ConfirmAlert روی همه چیز نشون داده میشه */}
            {alertInfo && (
                <ConfirmAlert
                    ...
                    onClose={() => {
                        setAlertInfo(null);
                        // اگر success بود، modal رو هم ببند
                        if (alertInfo.type === 'success') onClose();
                    }}
                />
            )}
