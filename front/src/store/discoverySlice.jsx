...
export const fetchScanHistory = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.detail || "Failed to load scan history"
      );
    }
  }
);
...
export const fetchPendingHosts = createAsyncThunk(
  ...
  async (scanId = null, { rejectWithValue }) => {
    try {
      const url = scanId
        ? `/api/discovery/pending?scan_id=${scanId}`
  ...
  async (hostId, { rejectWithValue }) => {
    try {
      const response = await api.get(
        `/api/discovery/hosts/${hostId}/check-matches`
      );
      return response.data;
    } catch (error) {
      return rejectWithValue(
  ...
  async ({ hostId, action, assetId, assetData }, { rejectWithValue }) => {
    try {
      ...
      const response = await api.post(
        `/api/discovery/hosts/${hostId}/approve`,
        requestBody
      );
...
export const previewDiscoveryApplication = createAsyncThunk(
  ...
  async ({ hostId, assetId }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.detail || "Failed to preview application"
      );
    }
  }
);
...
export const applyDiscoveryWithMode = createAsyncThunk(
  ...
  async (
    { hostId, mode, assetId, assetName, assetTypeId, locationId, ownerId },
    { rejectWithValue }
  ) => {
    try {
      const requestBody = {
      ...
      const response = await api.post(
        `/api/discovery/hosts/${hostId}/apply`,
        requestBody
      );
...
export const bulkApproveHosts = createAsyncThunk(
  ...
  async (
    { hostIds, defaultAssetTypeId, defaultLocationId, defaultOwnerId },
    { rejectWithValue }
  ) => {
    try {
      const requestBody = {
      ...
      if (defaultLocationId)
        requestBody.default_location_id = defaultLocationId;
      if (defaultOwnerId) requestBody.default_owner_id = defaultOwnerId;

      const response = await api.post(
        "/api/discovery/bulk-approve",
        requestBody
      );
      return response.data;
    } catch (error) {
...
export const addPortsToAsset = createAsyncThunk(
  ...
  async ({ assetId, ports, scanId }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.detail || "Failed to add ports"
      );
    }
  }
);
...
export const overwriteAssetPorts = createAsyncThunk(
  ...
  async ({ assetId, ports, scanId }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.detail || "Failed to overwrite ports"
      );
    }
  }
);
...
export const fetchAssetPorts = createAsyncThunk(
  ...
  async (assetId, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.detail || "Failed to fetch asset ports"
      );
    }
  }
);
...
export const deletePort = createAsyncThunk(
  ...
  async (portId, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.detail || "Failed to delete port"
      );
    }
  }
);
    ...
    builder
      ...
      .addCase(approveHost.fulfilled, (state, action) => {
        ...
        if (action.payload.createdAsset) {
          if (
            !state.discoveryCreatedAssetIds.includes(
              action.payload.createdAsset
            )
          ) {
        ...
        // Remove from pending
        state.pendingHosts = state.pendingHosts.filter(
          (h) => h.id !== action.payload.hostId
        );
        state.selectedHost = null;
        state.matchResults = null;
      })
      ...
      .addCase(deletePort.fulfilled, (state, action) => {
        state.loading.ports = false;
        state.assetPorts = state.assetPorts.filter(
