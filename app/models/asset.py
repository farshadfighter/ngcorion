...
class Asset(Base):
    ...
    Display Views (Asset List):
    --------------------------
    1. Overview: Basic asset information
    2. Network and System: Technical details
    3. Location and Ownership: Location and owner info
    4. Open Port and Service: Network ports (separate table)
    5. Security/Risk/Audit: Security status
    
    Attributes:
        id: Unique identifier (auto-generated) - asset_id in forms
        asset_name: Name of the asset
        hostname: Network hostname
        asset_type_id: FK to asset_types
        asset_role: Role/purpose of asset
        ...
        owner_id: FK to asset_owners
        status: Asset status (active, standby, etc.)
        confidentiality_level: Data classification level
        risk_level: Risk assessment level
        last_audit_date: Last audit/review date
        last_patch_date: Last security patch date
        asset_value: Financial value
        description: Additional notes
        user_id: FK to users (data isolation)
