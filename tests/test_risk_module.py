...
PDF model recap (source of truth):
    RiskScore = AC*0.20 + AR*0.20 + AZ*0.15 + OP*0.10 + AF*0.25 + HF*0.10
Finding severity weights: low 1, medium 4, high 7, critical 10.
Open-port points:         low 1, medium 3, high 5, critical 10; OP = min(100, sum).
AF = 100 * (failed applicable weight) / (all applicable weight).
HF = 100 * (failed-with-fix weight) / (all applicable weight)  [additive, +10%].
Hardening does NOT remove a finding from AF (Strict PDF).
Risk levels: 0-20 informational | 21-40 low | 41-60 medium | 61-80 high | 81-100 critical.
>>>>>>> 82dcfbd (debuge risk with last doc)
"""
...
=======
            updates={"risk_level_medium_threshold": 70},  # >= high (60)
>>>>>>> 82dcfbd (debuge risk with last doc)
